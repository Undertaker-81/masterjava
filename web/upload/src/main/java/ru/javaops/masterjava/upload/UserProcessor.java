...
public class UserProcessor {
    ...
    public List<FailedEmails> process(final StaxStreamProcessor processor, Map<String, City> cities, int chunkSize) throws XMLStreamException, JAXBException {
        ...
        Map<User, List<String>> mapUserGroups = new HashMap<>();
        int id = userDao.getSeqAndSkip(chunkSize);
        List<User> chunk = new ArrayList<>(chunkSize);
        val unmarshaller = jaxbParser.createUnmarshaller();
        List<FailedEmails> failed = new ArrayList<>();
        final Map<String,Group> mapAllGroups = StreamEx.of(groupDao.getAll()).toMap(Group::getName, identity());
        ...
        while (processor.doUntil(XMLEvent.START_ELEMENT, "User")) {
            ...
            if (groupRef != null){
                groups = Splitter.on(' ').splitToList(nullToEmpty(groupRef));
            }

            boolean groupsIsExistinDB = true;
            if (groups.size() > 0)
              groupsIsExistinDB = groups.stream().allMatch(mapAllGroups::containsKey);
            ...
            if (cities.get(cityRef) == null) {
                ...
                if (!groupsIsExistinDB){
                    ...
                    mapUserGroups.put(user, groups);
                    //  groupProcessor.process(user, groups);
                    chunk.add(user);
        ...
        chunkFutures.forEach((emailRange, future) -> {
            try {
                ...
                userDao.getAll().forEach(user -> {
                    if (mapUserGroups.containsKey(user)) {
                        groupProcessor.process(user, mapUserGroups.get(user));
                    }
                });
